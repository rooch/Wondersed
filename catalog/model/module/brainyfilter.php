 ...
 * 
 * BrainyFilter 2.2.2, May 29, 2014 / brainyfilter.com
...
class ModelModuleBrainyFilter extends Model 
{
	...
	public function __construct($registry) {
        ...
        if (isset($this->request->get['bfilter'])) {
            $raw = $this->_parseBFilterParam();
        }
        $this->request->get['bfilterraw'] = json_encode($raw);
        
		if ($this->attr_setting['price_filter']) {
			if (isset($raw['higher'])) {
				$this->higher = $raw['higher'];
			}
			
			if (isset($raw['lower'])) {
				$this->lower = $raw['lower'];
			}
		...
		if (isset($raw['attr'])) {
			$this->attributeValue = $raw['attr'];
		}
		if (isset($raw['brand'])) {
			$this->manufacturerValue = $raw['brand'];
		}
		if (isset($raw['stock'])) {
			$this->stockStatusValue = $raw['stock'];
		}
		if (isset($raw['rating'])) {
			$this->ratingValue = $raw['rating'];
		}
		if (isset($raw['option'])) {
			$this->optionValue = $this->_findOptionIds($raw['option']);
		}
	...
	public function prepareQueryForCategory($sql) 
	{
        ...
        // some opencart modifications (e.g. ocstore) can contain in the sql query
        // additional key words such as DISTINCT and/or SQL_CALC_FOUND_ROWS
        $distinct = preg_replace("/(^SELECT[\s]+)((DISTINCT[\s]+)?(SQL_CALC_FOUND_ROWS\s)?)(.*)$/mi", '$2', $sql);
        
		$sql = preg_replace("/(^SELECT[\s]+)((DISTINCT[\s]+)?(SQL_CALC_FOUND_ROWS\s)?)?(p.product_id,[\s]+)?(.*)$/mi", 
                "$1$5 " . $this->_columnsTax() . " p.price, $6", $sql);
		// add filter terms
		if (!empty($filter->terms) || !empty($filter->agregatedTerms)) {
			...
			if (!empty($filter->agregatedTerms)) {
				$sql = "SELECT " . $distinct . " * FROM (" . $sql . ") AS tt WHERE " . $filter->agregatedTerms . $limit;
			} else {
				$sql = "SELECT " . $distinct . " * FROM (" . $sql . ") AS tt " . $limit;
			}
		}
...
//		echo $sql;
	...
	private function _parseBFilterParam()
	{
		...
		foreach ($params as $param) {
			...
			if ($paramName == 'price') {
				...
				if ((int)$pValues[0] > 0) {
					$rawArr['lower']  = $pValues[0];
				}
				if ((int)$pValues[1] > 0) {
					$rawArr['higher'] = $pValues[1];
				}
				...
				$rawArr['rating'] = $values;
			} elseif ($paramName == 'option') {
				$rawArr['option'] = $values;
			} else {
				$p  = explode('-', $paramName);
				$id = (int)$p[0];

				if (count($values) && $id > 0) {
					$rawArr['attr'][$id] = $values;
				}
			}
		}
	...
	{	
		...
		if (count($this->stockStatusValue)) {
			...
			foreach ($this->stockStatusValue as $id) {
                if ($id == $this->attr_setting['stock_status_id']) {
                    $stockTermsArr[] = "p.quantity > 0";
                } else {
                    $stockTermsArr[] = "(p.stock_status_id = '" . $this->db->escape($id) . "' AND p.quantity = 0)";
                }
			}
	...
	private function _columnsTax()
	{
        ...
        if ($this->config->get('config_country_id') && $this->config->get('config_zone_id')) {
            $arr[] = "tr1.based = 'store' AND z2gz.country_id = '" . (int)$this->config->get('config_country_id') . "' AND z2gz.zone_id IN ('0', '" . (int)$this->payment_address[' zone_id '] . "')";
        }
        $terms = '((' . implode(') OR (', $arr) . '))';
        $terms .= " AND tr2cg.customer_group_id = '" . $customerGroupId . "'";

		$fixedTax   = "(SELECT SUM(rate) FROM " . DB_PREFIX . "tax_rule tr1 LEFT JOIN " . DB_PREFIX . "tax_rate tr2 ON (tr1.tax_rate_id = tr2.tax_rate_id) INNER JOIN " . DB_PREFIX . "tax_rate_to_customer_group tr2cg ON (tr2.tax_rate_id = tr2cg.tax_rate_id) LEFT JOIN " . DB_PREFIX . "zone_to_geo_zone z2gz ON (tr2.geo_zone_id = z2gz.geo_zone_id) LEFT JOIN " . DB_PREFIX . "geo_zone gz ON (tr2.geo_zone_id = gz.geo_zone_id) WHERE tr1.tax_class_id = p.tax_class_id AND tr2.type = 'F' AND {$terms}) AS fixed_tax";
		$percentTax = "(SELECT SUM(rate) FROM " . DB_PREFIX . "tax_rule tr1 LEFT JOIN " . DB_PREFIX . "tax_rate tr2 ON (tr1.tax_rate_id = tr2.tax_rate_id) INNER JOIN " . DB_PREFIX . "tax_rate_to_customer_group tr2cg ON (tr2.tax_rate_id = tr2cg.tax_rate_id) LEFT JOIN " . DB_PREFIX . "zone_to_geo_zone z2gz ON (tr2.geo_zone_id = z2gz.geo_zone_id) LEFT JOIN " . DB_PREFIX . "geo_zone gz ON (tr2.geo_zone_id = gz.geo_zone_id) WHERE tr1.tax_class_id = p.tax_class_id AND tr2.type = 'P' AND {$terms}) AS percent_tax";
